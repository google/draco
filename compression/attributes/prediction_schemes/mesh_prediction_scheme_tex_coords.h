...
//
#ifndef DRACO_COMPRESSION_ATTRIBUTES_PREDICTION_SCHEMES_MESH_PREDICTION_SCHEME_TEX_COORDS_H_
...
template <typename DataTypeT, class TransformT, class MeshDataT>
bool MeshPredictionSchemeTexCoords<DataTypeT, TransformT, MeshDataT>::Encode(
  ...
  for (int p = this->mesh_data().data_to_corner_map()->size() - 1; p >= 0;
    ...
    ComputePredictedValue<true>(corner_id, in_data, p);
...
bool MeshPredictionSchemeTexCoords<DataTypeT, TransformT, MeshDataT>::Decode(
  ...
  for (int p = 0; p < corner_map_size; ++p) {
    ...
    const int dst_offset = p * num_components;
    this->transform().ComputeOriginalValue(predicted_value_.get(), in_corr,
...
template <typename DataTypeT, class TransformT, class MeshDataT>
bool MeshPredictionSchemeTexCoords<DataTypeT, TransformT, MeshDataT>::
...
template <typename DataTypeT, class TransformT, class MeshDataT>
template <bool IS_ENCODER>
void MeshPredictionSchemeTexCoords<DataTypeT, TransformT, MeshDataT>::
