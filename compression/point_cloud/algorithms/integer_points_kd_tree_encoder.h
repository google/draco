...
class IntegerPointsKdTreeEncoder {
 ...
 private:
  ...
  template <class RandomAccessIteratorT>
  void OctreeEncode(RandomAccessIteratorT begin, RandomAccessIteratorT end,
                    PointDiT old_base, std::array<uint32_t, D> levels,
                    uint32_t last_axis);

  class OctreeLess {
   public:
    OctreeLess(int axis, uint32_t value) : axis_(axis), value_(value) {}
...
bool IntegerPointsKdTreeEncoder<PointDiT, compression_level_t>::EncodePoints(
  ...
  OctreeEncode(begin, end, PointTraits<PointDiT>::Origin(),
...
uint32_t IntegerPointsKdTreeEncoder<PointDiT, compression_level_t>::GetAxis(
  ...
  // for the split by keeping as many points as possible bundeled.
...
template <class PointDiT, int compression_level_t>
template <class RandomAccessIteratorT>
void IntegerPointsKdTreeEncoder<PointDiT, compression_level_t>::OctreeEncode(
  ...
  while (!status_q.empty()) {
    ...
    const RandomAccessIteratorT split =
        std::partition(begin, end, OctreeLess(axis, new_base[axis]));
