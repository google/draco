...
void RAnsSymbolEncoder<max_symbol_bit_length_t>::StartEncoding(
  ...
  // Offset the encoding by sizeof(buffer_offset_). We will use this memory to
  // store the number of encoded bytes.
  ans_.write_init(data + buffer_offset_ + sizeof(buffer_offset_));
}

template <int max_symbol_bit_length_t>
void RAnsSymbolEncoder<max_symbol_bit_length_t>::EndEncoding(
    EncoderBuffer *buffer) {
  const int64_t bytes_written = ans_.write_end();
  // Store the size of the encoded data.
  memcpy(const_cast<char *>(buffer->data()) + buffer_offset_, &bytes_written,
         sizeof(bytes_written));
  // Resize the buffer to match the number of encoded bytes.
  buffer->Resize(buffer_offset_ + bytes_written + sizeof(buffer_offset_));
}
