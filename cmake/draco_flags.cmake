...
# Exhaustively add/remove flags for specified sources. Attempts to update all
# configurations for the sources as determined by the current CMake generator.
#
# draco_replace_compiler_flags_for_sources(SOURCES <sources>
#                                          FLAGS_ADD <flags>
#                                          FLAGS_REMOVE <flags>)
macro(draco_replace_compiler_flags_for_sources)
  unset(flags_SOURCES)
  unset(flags_FLAGS_ADD)
  unset(flags_FLAGS_REMOVE)
  unset(optional_args)
  unset(single_value_args)
  set(multi_value_args SOURCES FLAGS_ADD FLAGS_REMOVE)
  cmake_parse_arguments(flags "${optional_args}" "${single_value_args}"
                        "${multi_value_args}" ${ARGN})

  if(NOT (flags_SOURCES AND (flags_FLAGS_ADD OR flags_FLAGS_REMOVE)))
    draco_die("draco_set_compiler_flags_for_sources: SOURCES and FLAGS_ADD "
              "or FLAGS_REMOVE required.")
  endif()

  #if(DRACO_VERBOSE)
    message("=================")
    message("----draco_replace_compiler_flags_for_sources")
    message("CMAKE_GENERATOR=${CMAKE_GENERATOR}")
    message("CMAKE_CXX_FLAGS=${CMAKE_CXX_FLAGS}")
    message("CMAKE_CONFIGURATION_TYPES=${CMAKE_CONFIGURATION_TYPES}")
    message("SOURCES=${flags_SOURCES}")
    message("FLAGS_ADD=${flags_FLAGS_ADD}")
    message("FLAGS_REMOVE=${flags_FLAGS_REMOVE}")
  #endif()
  ...
  if(configs STREQUAL "")
    ...
    list(TRANSFORM configs TOUPPER)
  endif()

  #if(DRACO_VERBOSE)
    foreach(config ${configs})
      message("CMAKE_CXX_FLAGS_${config}=${CMAKE_CXX_FLAGS_${config}}")
    endforeach()
  #endif()

  foreach(source ${flags_SOURCES})
    #if(DRACO_VERBOSE GREATER 1)
      message("====")
      message("source=${source}")
    #endif()

    get_source_file_property(source_flags "${source}" CMAKE_CXX_FLAGS)
    if(source_flags STREQUAL "" AND NOT (CMAKE_CXX_FLAGS STREQUAL ""))
      set_source_files_properties(
          "${source}" PROPERTIES CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS})
    endif()

    #if(DRACO_VERBOSE GREATER 1)
      message("base flags=${source_flags}")
    #endif()

    foreach(config ${configs})
      get_source_file_property(source_flags ${source} CMAKE_CXX_FLAGS_${config})

      if(source_flags STREQUAL NOTFOUND)
        set(source_flags "")
      endif()

      if(source_flags STREQUAL ""
          AND NOT (CMAKE_CXX_FLAGS_${config} STREQUAL ""))
      ...
      #if(DRACO_VERBOSE GREATER 1)
        message("${config} flags=${source_flags}")
      #endif()

      set_source_files_properties(
          ${source} PROPERTIES CMAKE_CXX_FLAGS_${config} "${source_flags}")
    endforeach()
