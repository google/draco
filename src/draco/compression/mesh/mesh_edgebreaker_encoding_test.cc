...
TEST_F(MeshEdgebreakerEncodingTest, RejectExcessiveSymbols) {
  // Tests that a crafted buffer with num_encoded_symbols exceeding the remaining
  // bit budget is cleanly rejected without uncontrolled memory allocation
  // (fixes #1169).
  const uint8_t kCorruptedData[] = {
      0x44, 0x52, 0x41, 0x43, 0x4f, 0x01, 0x00, 0x01, 0x01, 0x5d, 0x01,
      0x01, 0x00, 0x00, 0x7d, 0x00, 0x00, 0x33, 0x33, 0x33, 0x33, 0x33,
