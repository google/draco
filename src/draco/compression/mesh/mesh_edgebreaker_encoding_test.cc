...
class MeshEdgebreakerEncodingTest : public ::testing::Test {
 protected:
  ...
  void TestMesh(Mesh *mesh, int compression_level) {
    ...
    DRACO_ASSERT_OK(
        decoder.DecodeStep1(dec_options, &dec_buffer, decoded_mesh.get()));
    DRACO_ASSERT_OK(
        decoder.DecodeStep2());
...
TEST_F(MeshEdgebreakerEncodingTest, TestDecoderReuse) {
  ...
  DecoderOptions dec_options;
  DRACO_ASSERT_OK(
      decoder.DecodeStep1(dec_options, &dec_buffer, decoded_mesh_0.get()));
  DRACO_ASSERT_OK(
      decoder.DecodeStep2());
  dec_buffer.Init(buffer.data(), buffer.size());
  std::unique_ptr<Mesh> decoded_mesh_1(new Mesh());
  DRACO_ASSERT_OK(
      decoder.DecodeStep1(dec_options, &dec_buffer, decoded_mesh_1.get()));
  DRACO_ASSERT_OK(
      decoder.DecodeStep2());
